...
class Kauppa:
    def __init__(
        self, varasto: Varasto, pankki: Pankki, viitegeneraattori: Viitegeneraattori
    ):
        self._varasto = varasto
        self._pankki = pankki
        self._viitegeneraattori = viitegeneraattori
        self._kaupan_tili = "33333-44455"
        self._ostoskori: Ostoskori | None = None

    def aloita_asiointi(self):
        self._ostoskori = Ostoskori()

    def poista_korista(self, id: str):
        tuote = self._varasto.hae_tuote(id)
        if not tuote:
            return
        poistetut = self._ostoskori.poista(tuote)
        for _ in range(poistetut):
            self._varasto.palauta_varastoon(tuote)

    def lisaa_koriin(self, id: str):
    ...
    def tilimaksu(self, nimi, tili_numero):
        ...
        return self._pankki.tilisiirto(
            nimi, viite, tili_numero, self._kaupan_tili, summa
        )
